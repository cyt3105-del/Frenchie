        ...
        <View className="bg-surface rounded-3xl p-6 flex-1 border border-border shadow-sm">
          ...
          <View className="flex-row justify-between items-center mb-6">
            ...
            <View className="flex-row items-center gap-2">
              {forgotCount > 0 && (
                <View className={`px-3 py-1 rounded-full ${recentlyForgotten ? 'bg-red-100' : 'bg-warning/20'}`}>
                  <Text className={`text-xs font-semibold ${recentlyForgotten ? 'text-red-600' : 'text-warning'}`}>
                    {recentlyForgotten ? '🔄 Review' : `${forgotCount}x forgot`}
                  </Text>
                </View>
              )}
              {/* Speaker button */}
